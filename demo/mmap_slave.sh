#!/bin/bash

cd ../user_program

echo "Demo mmap sample_input_1"
cd ../output/sample_output_1
sudo ../../user_program/slave 10 mmap_received_file_1 mmap_received_file_2 mmap_received_file_3 mmap_received_file_4 mmap_received_file_5 mmap_received_file_6 mmap_received_file_7 mmap_received_file_8 mmap_received_file_9 mmap_received_file_10 mmap 127.0.0.1
for i in {1..10}
do
  diff -a mmap_received_file_$i ../../input/sample_input_1/target_file_$i > /dev/null 2>&1
  if [ $? -eq 0 ]
  then
    echo "target_file_${i} and mmap_received_file_${i} are similar."
  fi
done

echo "Demo mmap sample_input_2"
cd ../sample_output_2
sudo ../../user_program/slave 1 mmap_received_file mmap 127.0.0.1
diff -a mmap_received_file ../../input/sample_input_2/target_file > /dev/null 2>&1
if [ $? -eq 0 ]
then
  echo "target_file and mmap_received_file are similar."
fi

echo "Demo mmap my_input"
cd ../my_output
sudo ../../user_program/slave 5 out_1 out_2 out_3 out_4 out_5 mmap 127.0.0.1
for i in {1..5}
do
  diff -a out_$i ../../input/my_input/in_$i > /dev/null 2>&1
  if [ $? -eq 0 ]
  then
    echo "in_${i} and out_${i} are similar."
  fi
done
